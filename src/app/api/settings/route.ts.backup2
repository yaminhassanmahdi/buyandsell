...
export async function GET(request: Request) {
  try {
    ...
    const settings = {
      ...
      logoUrlHorizontal: result.logo_url_horizontal || result.logo_url || '',
      logoUrlVertical: result.logo_url_vertical || result.logo_url || '',
  ...
  try {
    ...
    const query = `
      INSERT INTO business_settings (
        setting_key, app_name, logo_url, logo_url_horizontal, logo_url_vertical,
        favicon_url, primary_color, secondary_color, available_currencies, 
        default_currency_code, google_client_id, google_client_secret
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON DUPLICATE KEY UPDATE
        app_name = VALUES(app_name),
        logo_url = VALUES(logo_url),
        logo_url_horizontal = VALUES(logo_url_horizontal),
        logo_url_vertical = VALUES(logo_url_vertical),
    ...
    await executeQuery(query, [
      ...
      settings.appName,
      settings.logoUrl || '',
      settings.logoUrlHorizontal || settings.logoUrl || '',
      settings.logoUrlVertical || settings.logoUrl || '',
      settings.faviconUrl || '',
