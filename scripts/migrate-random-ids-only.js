  ...
  try {
    ...
    for (const category of categories) {
      if (isRandomId(category.id)) {
        ...
        // Ensure uniqueness
        while (mappings.categories.has(finalId)) {
    ...
    for (const user of users) {
      if (isRandomId(user.id)) {
        ...
        // Ensure uniqueness
        while (mappings.users.has(finalId)) {
    ...
    for (const product of products) {
      if (isRandomId(product.id)) {
        ...
        // Ensure uniqueness
        while (mappings.products.has(finalId)) {
          finalId = `${baseId}${counter}`;
          counter++;
        }
    ...
    for (const order of orders) {
      if (isRandomId(order.id)) {
        ...
        // Ensure uniqueness
        while (mappings.orders.has(finalId)) {
          finalId = `${baseId}-${counter}`;
          counter++;
        }
    ...
    if (categoryCount === 0 && userCount === 0 && productCount === 0 && orderCount === 0) {
